...
> ⚠️ **הערה על עדכניות**: מסמך זה נכתב כשמוד מכלולים היה בעל pipeline נפרד
> עם ניתוח קשרי אבא/בן ועץ מוצר. **החל מ-24/04/2026** מוד מכלולים רץ על
> אותו pipeline של מוד בודד (`core/extractor.extract_drawing`) לכל PDF
> בנפרד, עם דפדוף ודוחות מאוחדים בסוף. אין יותר `core/assembly.py`,
> `core/assembly_prompts.py`, `prompts/assembly/`, או ניתוח קשרים.
> ראה [CHANGELOG.md](CHANGELOG.md) לפרטים. חלקים ממסמך זה עדיין מתארים
> את המצב הישן.

> אפליקציית Streamlit לחילוץ אוטומטי של מידע משרטוטים הנדסיים (PDF) באמצעות
> Azure OpenAI Vision / Reasoning. תומכת בשני מצבי עבודה: **שרטוט בודד** —
> ניתוח PDF יחיד עם התאמה למאגר מאסטרים; ו-**מכלולים מרובים** — ניתוח מספר
> PDFs ברצף (אותו pipeline לכל אחד, ללא קשר ביניהם), עם דפדוף בין
> התוצאות ודוחות JSON/Excel/PDF מאוחדים.
